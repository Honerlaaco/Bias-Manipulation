...
def parse_args():
    ...
    parser.add_argument("--EXP_NAME", type=str, default='gowalla_FedAVBG')
    parser.add_argument("--MODEL_TYPE", type=str, default="MF")
    parser.add_argument("--DATA", type=str, default="gowalla", choices=["ml", "gowalla"])
    ...
    parser.add_argument("--SAVE_ROUND", type=int, default=2000)
...
for uid in user_data:
    user_data[uid] = sorted(user_data[uid], key=lambda x: x[-1])   # 排序
...
if os.path.exists(cache_path):                                       #加载数据集
